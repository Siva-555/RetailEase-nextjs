...
export async function GET(req: NextRequest) {
  try {
    ...
    for (let i = 0; i < days; i++) {
      ...
      countsMap[formattedDate] = { total: 0, paid: 0, unpaid: 0 };
    }

    allBills.forEach((bill) => {
      const date = formatInTimeZone(bill.created_date, "UTC", "yyyy-MM-dd");
      if (countsMap[date]) {
        countsMap[date].total += 1;
        if (bill.paid) countsMap[date].paid += 1;
        else countsMap[date].unpaid += 1;
      }
    });
    ...
    console.error("Error order stats", error);
    return NextResponse.json(
      { error: "Error order stats", status: "fail", },
      { status: 500 }
    );
  }
}
